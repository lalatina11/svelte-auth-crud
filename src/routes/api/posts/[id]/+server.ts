...
export const GET = async ({ params, cookies }: RequestEvent & { params: { id: number } }) => {
    try {
        ...
        }).from(postTable).leftJoin(userTable, eq(userTable.id, postTable.userId)).where(eq(postTable.id, id))
        ...
        return json({ error: false, data: { likes, comments, isLiked: !!likes.some(like => like.userId === currentUserId), likeCount: likes.length, post }, message: "ok" }, { status: 200 })
    } catch (error) {
